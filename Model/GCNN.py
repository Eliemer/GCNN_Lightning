...
from Datasets.tools import get_subsets, accuracy, precision, recall
...
class GCNN(pl.LightningModule):
	...
	def test_dataloader(self):
		return DataLoader(self.subsets[1], batch_size=self.batch_size)

	def val_dataloader(self):
		return DataLoader(self.subsets[2], batch_size=self.batch_size)
	...
	def __init__(self, config_dict):
        ...
        for i in range(config_dict['nb_conv_layers']):
            self.conv_layers[f'kernel_{i}'] = GraphKernels(
                ...
                batch_size=config_dict['batch_size'],             # X.size()[0],
                training=None)
            self.conv_layers[f'conv_{i}'] = GraphConv(
                nb_filters=config_dict['nb_filters'],
                nb_nodes=for_nb_nodes,               # X.size()[1],
                nb_features=for_nb_features,            # X.size()[2],
                support=config_dict['support'],
	...
	def forward(self, v, c, mask=None): # features, coordinates, adjacency, mask
        # print('###### start forward ######')
